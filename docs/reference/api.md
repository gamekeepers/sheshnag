...
The control plane's HTTP contract: what each endpoint does, who may call it, and
what comes back. This is a reference — look things up here rather than reading
it front to back.
...
| `POST` | `/v1/orgs/{org_id}/api-keys` | owner/admin | **Create** a worker key; the raw `gk-…` is returned **once** and never again |
| `DELETE` | `/v1/orgs/{org_id}/api-keys/{key_id}` | owner/admin | Revoke a key — every daemon using it stops registering immediately |
...
- **`vram_total_gb` is `null`** for anonymous callers, and for everyone while
  fewer than `MIN_WORKERS_FOR_VRAM` (3) workers are online — on a thin pool
  "141 GB" names a specific machine, which is the disclosure the aggregate
  exists to prevent.
...
poll time. See **[Model catalogue](model-catalogue.md)** for
...
The backend enforces this rather than merely describing it —
`VALID_TRANSITIONS` in `backend/routers/workers.py` rejects anything else:

| From | May become |
|---|---|
| `validating` | `validated`, `failed` |
| `validated` | `in_progress` |
| `in_progress` | `completed`, `failed`, `validated` *(the last is a requeue)* |
| `completed` | — terminal |
| `failed` | — terminal |

**There are exactly five.** `cancelling`, `cancelled`, `expired` and
`finalizing` exist in OpenAI's batch API and the dashboard styles badges for
them, but this backend never produces them — there is no cancel endpoint.

**Requeue** (`requeue_or_fail_batch` in `backend/sweeper.py`) increments
`attempts`, deletes the batch's assignment so another worker can claim it, and
stores the error truncated to 2000 characters. On the third attempt
(`MAX_BATCH_ATTEMPTS = 3`) the batch goes terminal: `failed`, `completed_at`
stamped, and `request_counts_failed` set to the total.

Two things trigger it — a worker calling `POST /workers/report-failure`, and the
sweeper finding a worker that has stopped heartbeating
...
`validating`, and the batch reaches `validated` or `failed` on its own. Watch it
with `GET /v1/batches/{batch_id}/events` (SSE) rather than polling.

Each error carries a `code` (`missing_field`, `invalid_json`,
`duplicate_custom_id`, `unknown_fields`, `unsupported_model`, …), the `line` it
occurred on, and the `field` where applicable — `null` for whole-line JSON parse
failures.

**Stored errors are capped at `MAX_STORED_ERRORS = 100`**
(`backend/services/batch_validator.py`), but the reported total is the true
count. A batch can therefore report thousands of errors while listing a hundred;
that is the cap, not the truth about the file.
